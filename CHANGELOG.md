...
- 登录与注册页面同步切换为与 Header、侧边栏和业务卡片一致的浅色玻璃设计语言，移除独立深色宣传面板。
- 品牌区、能力卡、表单卡、模式切换和状态条统一使用柔和青绿色、细边框、玻璃表面与相同圆角层级。
- 深青色仅用于品牌图标、登录 / 注册选中态和主提交按钮，保持与后台主要操作一致的视觉语义。
- 登录与注册按钮新增提交中反馈，输入框补充必填和浏览器自动填充语义。

### 侧边栏视觉与导航体验升级

- 侧边栏重构为与 Header、页面介绍和业务卡片一致的浅色玻璃运营控制台，新增柔和渐变、网格纹理、品牌状态灯和更精细的层次阴影。
- 侧边栏仅对当前选中项使用深青色主操作强调，与 Header 新建订单按钮和账号图标保持同一视觉语义。
- 导航按“工作台、业务运营、资源调度、财务与治理”分组，降低长列表查找成本。
- 当前页面选中态改为克制的渐变高亮、侧边光带和图标强调，嵌套页面也会正确高亮所属一级模块。
- 导航区域改为独立滚动，品牌区和底部角色状态卡保持固定，改善较矮屏幕和移动端抽屉中的使用体验。
- 移除固定的虚假“今日运转率”，底部改为真实角色、在线状态、权限同步状态和快捷入口；无系统设置权限的角色不会看到设置入口。
