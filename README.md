...
- `登录与注册入口` 已同步为后台统一的浅色玻璃设计语言，品牌概览、表单、模式切换、提示卡和主按钮与 Header、侧边栏及业务卡片保持一致，并补充提交中反馈和表单自动填充语义。
- `全局侧边栏` 已升级为与 Header 和业务卡片统一的浅色玻璃分组导航：按工作台、业务运营、资源调度、财务治理组织入口，使用统一圆角、细边框、柔和阴影和深青色主操作强调，并支持嵌套路由高亮、独立滚动、真实角色状态和权限感知快捷入口。
...
- `订单管理` 已新增归档与历史检索能力：已完成 / 已取消订单可归档保存，活跃工作台默认隐藏归档订单；历史区支持按服务日期范围、订单号、客户、行程内容、归档摘要和关键词检索，并可查看只读详情或移出归档后继续编辑。
- `订单管理` 页面已重新构造成操作优先的订单工作台：移除顶部介绍、摘要卡、说明卡和七日排班展示，首屏直接处理创建、编辑、删除订单；订单详情、状态流转、调度与成本维护已收进二级编辑界面，近期运营提醒和待审批队列保留在工作台下方。
- `Dashboard Header` 的运营提醒弹层已提升到页面最高交互层，修复提醒面板被统计卡片、玻璃背景或主内容区域压住的问题；搜索下拉也同步提升层级并改为更清晰的不透明面板。
- `演示数据` 已重置为 2026-06 东京入境业务样例：旧 mock 订单与旧 seed 数据已替换为 8 个互相关联的订单、5 个客户、5 台车辆、5 名司机、5 名导游、5 份报价、成本拆分、回款和供应商付款，方便用现有订单、日历、车辆、司机、财务和利润功能联动测试。
- `全局 UI` 已补充文字防溢出规则，按钮、徽标、统计卡、摘要卡、SectionCard、StatStrip 和 DataTable 会更稳地处理长文本、邮箱、订单号和金额，避免文字撑破卡片。
- `运营日历` 已修复月份选择与当前月份读取问题，默认按东京日期显示当前月份，并支持上一月 / 下一月 / 月份输入；日历内可按选中日期新建订单、选择出发时间、编辑订单时间，并支持删除订单。
- `Dashboard 卡片` 已升级为真实可点击运营入口：顶部统计、运营驾驶舱、订单推进漏斗、今日重点提醒和协同建议都能进入对应模块，订单 / 报价 / 车辆页会自动按 `status` 参数切换筛选。
- `Dashboard Header` 中的顶部搜索、新建订单和通知铃铛已从静态展示升级为真实操作入口：搜索可跳转并过滤订单 / 客户 / 司机 / 报价单，新建订单会打开正式建单弹层，通知铃铛会显示近期运营提醒。
...
- `客户管理` 已升级为真实两级档案系统，一级查看客户类型、公司介绍、订单数量和合作待办，二级维护联系人、微信 / LINE、跟进、历史业务和合作需求任务。
...
- 司机模块已切换到真实工作台，支持微信 / LINE、月出勤天数、自定义颜色、默认车辆和事故记录维护
- 司机与车辆模块已接入共享月度排班表和每日匹配功能，可查看或修改每天的司机、线路与车辆
- 司机车辆匹配已接入同日撞单拦截，并在车辆留空时自动使用司机默认车辆
- 已隔离开发态与生产构建输出目录，减少每次验证后 UI 崩溃的风险
- Vercel 生产部署所需的站点 URL 配置已接入，Supabase 邮件确认会优先回跳到 `NEXT_PUBLIC_SITE_URL`
...
- 第二阶段新增 `/audit` 统一操作日志中心，管理员、运营和财务可筛选并追溯客户、订单、财务、账号和系统设置的关键改动。
- 客户合作任务现在支持分配负责人；完成任务时会自动记录完成时间和完成人，Dashboard 提醒也会显示负责人。
- 上线第二阶段前，请在 Supabase SQL Editor 再次执行最新版 [supabase/schema.sql](/Users/jiaxinli/Desktop/公司管理系统/supabase/schema.sql)，以新增合作任务负责人和完成留痕字段。
- 新增通用操作审计与财务作废机制：错误回款和供应商付款需要填写原因后作废，历史流水不会再被直接删除。
- 财务页新增最近操作记录，作废流水会从应收与现金流计算中排除，但继续保留供月底核查。
- Dashboard 已接入客户合作事项提醒；创建订单选择客户后会展示未完成要求，并把要求快照自动写入订单备注。
- 上线本次数据库结构前，请在 Supabase SQL Editor 重新执行最新版 [supabase/schema.sql](/Users/jiaxinli/Desktop/公司管理系统/supabase/schema.sql)；不要在生产环境执行会重置演示业务数据的 `supabase/seed.sql`。
